...
// Normalize Bengali characters for fuzzy comparison
...
/**
 * Intelligent Smart Search matching products across title, category, subcategory, sku, and synonyms.
 * @param {Array} products 
 * @param {string} query 
 * @returns {Array} Sorted matching products
 */
export function smartSearchProducts(products, query) {
  ...
  // Collect synonym expansion terms
  ...
  // Also include individual space-separated words from the original query
  ...
  for (const p of products) {
    ...
    for (const token of searchTokens) {
      if (token === normalizedQ) continue; // already evaluated
      if (normTitle.includes(token)) {
        score += 40;
      }
      if (normSub.includes(token)) {
        score += 35;
      }
